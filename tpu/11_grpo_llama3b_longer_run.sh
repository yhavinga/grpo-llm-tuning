...
CMD+=" --repo_id meta-llama/Llama-3.2-3B-Instruct" # Using Llama-3.2-1B for a faster longer run than 8B.
...
CMD+=" --total_batch_size 4"                      # Total effective batch size across all devices (adjust based on memory).
