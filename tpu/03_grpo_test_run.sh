...
# --- 3. Construct and Run the GRPO Test Command ---
echo "Constructing the GRPO test run command..."
...
# Parameters here are set for a *quick test run* on a small model.

CMD="python -m easydel.scripts.finetune.gsm8k_grpo"   # The EasyDeL module to execute.
CMD+=" --repo_id meta-llama/Llama-3.2-1B-Instruct"
...
CMD+=" --sharding_axis '1,1,-1,1'"                # ** Sharding Strategy **
                                                ...
                                                #               -> Seems required for Llama-3-8B `compute_refmodel_logps`.
...
CMD+=" --total_batch_size 4"                      # Total effective batch size across all TPU devices.
                                                # Increase for real training (e.g., 16, 32, 64 depending on memory).
...
CMD+=" --log_steps 10"                            # Log metrics every N steps.
CMD+=" --max_training_steps 50"                   # **TEST ONLY**: Limit total training steps for a quick run. Remove/increase for full training.
CMD+=" --max_evaluation_steps 10"                 # **TEST ONLY**: Limit evaluation steps.
CMD+=" --save_steps 100"                          # Save a checkpoint every N steps.
...
echo ""
echo "--- GRPO Test Run Command Submitted ---"
...
echo "Note: This was a test run with limited steps. For a full run, adjust parameters like max_training_steps, total_batch_size, learning_rate, etc."
